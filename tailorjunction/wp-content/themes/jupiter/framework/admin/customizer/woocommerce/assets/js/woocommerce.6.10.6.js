! function($) {
    $(document).ready(function() {
        ...
        variations_form.on("woocommerce_variation_has_changed", function(e) {
            ...
            $(".mk-single-product-badges").find(".onsale").hide(), button.is(".wc-variation-is-unavailable") ? $(".mk-single-product-badges").find(".mk-out-of-stock").show() : ($(".mk-single-product-badges").find(".mk-out-of-stock").hide(), variations_form.find(".price del").length && $(".mk-single-product-badges").find(".onsale").show())
        })
    })
}(jQuery), jQuery(document).ready(function($) {
        if ($("body").hasClass("woocommerce-checkout") && $(".woocommerce-checkout .mk-grid").hasClass("full-layout")) {
            var resizeTimer, mkFloatCheckoutSummeryTable = function(scroll) {
                var table = $("#customer_details .col-2");
                if (0 == table.length) return !1;
                var tableTop = 0,
                    tableRight = $(".woocommerce-billing-fields__field-wrapper").position().left;
                1 == scroll && (tableTop += window.pageYOffset), table.css({
                    top: tableTop,
                    right: tableRight,
                    visibility: "visible"
                })
        ...
        mkQuantityInput(), $(document.body).on("updated_cart_totals", function() {
            mkQuantityInput()
    ...
    function(factory) {
        ...
        Slick = function() {
            ...
            if (!0 === _.options.centerMode ? !0 === _.options.infinite ? (rangeStart = _.currentSlide + (_.options.slidesToShow / 2 + 1), rangeEnd = rangeStart + _.options.slidesToShow + 2) : (rangeStart = Math.max(0, _.currentSlide - (_.options.slidesToShow / 2 + 1)), rangeEnd = _.options.slidesToShow / 2 + 1 + 2 + _.currentSlide) : (rangeStart = _.options.infinite ? _.options.slidesToShow + _.currentSlide : _.currentSlide, rangeEnd = Math.ceil(rangeStart + _.options.slidesToShow), !0 === _.options.fade && (rangeStart > 0 && rangeStart--, rangeEnd <= _.slideCount && rangeEnd++)), loadRange = _.$slider.find(".slick-slide").slice(rangeStart, rangeEnd), "anticipated" === _.options.lazyLoad)
            ...
            var _ = this;
            if (!_.unslicked && (_.$slider.trigger("afterChange", [_, index]), _.animating = !1, _.slideCount > _.options.slidesToShow && _.setPosition(), _.swipeLeft = null, _.options.autoplay && _.autoPlay(), !0 === _.options.accessibility && (_.initADA(), !_.options.autoplay))) {
            ...
            if (index || (index = 0), _.slideCount <= _.options.slidesToShow) return void _.slideHandler(index, !1, !0);
            ...
            if (sync = sync || !1, !(!0 === _.animating && !0 === _.options.waitForAnimate || !0 === _.options.fade && _.currentSlide === index)) {
                ...
                if (!1 === _.options.infinite && !0 === _.options.centerMode && (index < 0 || index > _.slideCount - _.options.slidesToScroll)) return void(!1 === _.options.fade && (targetSlide = _.currentSlide, !0 !== dontAnimate ? _.animateSlide(slideLeft, function() {
                    _.postSlide(targetSlide)
                }) : _.postSlide(targetSlide)));
            ...
            var _ = this;
            null !== _.$dots && (_.$dots.find("li").removeClass("slick-active").end().find("button").attr({
                "aria-pressed": !1
        ...
        function mkWooStickySidebar() {
            mkStickyScrollUp(), $.scrollDetection({
                scrollUp: function() {
                    mkStickyScrollUp()
                },
                scrollDown: function() {
        ...
        function mkStickyScrollUp() {
            var top = 0;
            top += $(".mk-single-product-badges").outerHeight() / 2, top += $(".a-sticky .mk-header-holder").outerHeight() || 50, $(".entry-summary").css({
                top: top
            })
        }
        ...
        function mkStickyScrollDown() {
            var top = 20,
                sidebarHeight = $(".entry-summary").outerHeight(),
                viewportHeight = $(window).height();
            sidebarHeight > viewportHeight ? top = viewportHeight - sidebarHeight : top += $(".a-sticky .mk-header-holder").outerHeight() || 30, $(".entry-summary").css({
                top: top
            })
        }
        $.scrollDetection = function(options) {
            ...
            $(window).scroll(function() {
                ...
                cursorPosition > scrollPosition ? settings.scrollDown() : cursorPosition < scrollPosition && settings.scrollUp(), scrollPosition = cursorPosition
            })
        }, MK.utils.isMobile() || ($("body").on("mk:woo-info-sticky", mkWooStickySidebar), 0 !== $(".mk-info-sticky", ".single-product").length && mkWooStickySidebar())
    }), jQuery(document).ready(function($) {
        function mkAlignVariationsQuantity() {
            var $variations = $(".variations", ".summary"),
                $quantity = $(".mk-product-quantity", ".summary");
            if (!$variations.is(":visible")) return void $quantity.removeAttr("style");
            if ($("body").hasClass("mk-product-layout-8")) return void $quantity.removeAttr("style");
            var $variationLabel = $("td.label", $variations),
                $quantityLabelWidth = $(".mk-quantity-label", ".summary").width();
            $variationLabel.css({
                minWidth: $quantityLabelWidth
            });
            var variationLabelWidth = $variationLabel.outerWidth(),
                variationLabelPadding = $variationLabel.css("padding-left"),
                left = variationLabelWidth + parseInt(variationLabelPadding);
            $quantity.css({
                left: left,
                position: "absolute"
            })
        }
        var verticalStatus = !1,
            slidesToShow = 6;
        $(".single-product .product").hasClass("mk-product-orientation-vertical") && (verticalStatus = !0, slidesToShow = 3), $(".flex-control-nav").slick({
            infinite: !1,
            slidesToShow: slidesToShow,
            slidesToScroll: 1,
            vertical: verticalStatus,
            prevArrow: '<button class="slick-prev" aria-label="Previous" type="button">\t\t<svg fill="#333333" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"\t\t\t width="7.2px" height="12px" viewBox="0 0 7.2 12" style="enable-background:new 0 0 7.2 12;" xml:space="preserve">\t\t<path class="st0" d="M2.4,6l4.5-4.3c0.4-0.4,0.4-1,0-1.4c-0.4-0.4-1-0.4-1.4,0l-5.2,5C0.1,5.5,0,5.7,0,6s0.1,0.5,0.3,0.7l5.2,5\t\t\tC5.7,11.9,6,12,6.2,12c0.3,0,0.5-0.1,0.7-0.3c0.4-0.4,0.4-1,0-1.4L2.4,6z"/>\t\t</svg>\t\t</button>',
        ...
        if (mkAlignVariationsQuantity(), $(document.body).on("mk-woo-align-quantity", mkHandleAlign), $(window).on("resize", mkHandleAlign), $.isFunction($.fn.zoom) && wc_single_product_params.zoom_enabled) {
            var zoomTarget = $(".woocommerce-product-gallery__image"),
                galleryWidth = zoomTarget.parents(".flex-viewport").width(),
                zoomEnabled = !1;
            if ($(zoomTarget).each(function(index, target) {
                    if ($(target).find("img").data("large_image_width") > galleryWidth) return zoomEnabled = !0, !1
                }), zoomEnabled) {
                var zoom_options = {
                    touch: !1
                };
                "ontouchstart" in window && (zoom_options.on = "click"), zoomTarget.trigger("zoom.destroy"), zoomTarget.zoom(zoom_options)
            }
        }
    });